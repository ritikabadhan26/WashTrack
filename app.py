...
# Default staff login seeded automatically on first run.
# Change this password before real-world/production use!
DEFAULT_STAFF_USERNAME = "staff"
DEFAULT_STAFF_PASSWORD = "laundry123"
...
def init_db():
    ...
    # Seed one default staff account so the admin dashboard is reachable
    # on a fresh install without needing a separate seeding step.
    create_user(
...
def create_user(username, password, role, full_name, room_number=None, phone=None):
    """
    Create a new login account (student or staff).
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            """INSERT INTO users (username, password_hash, role, full_name, room_number, phone)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (username, generate_password_hash(password), role, full_name, room_number, phone),
        )
        conn.commit()
...
def get_user_by_username(username):
    """Fetch a single user account by username (case-sensitive)."""
    conn = get_db_connection()
    try:
        row = conn.execute(
...
def authenticate(username, password):
    ...
    Returns the user dict on success, or None if invalid.
    """
...
def create_order(student_name, room_number, phone, pickup_slot, items, user_id=None):
    """
    ...
    user_id: the logged-in student's account id (for ownership tracking).
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            """INSERT INTO orders (user_id, student_name, room_number, phone, pickup_slot, status)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (user_id, student_name, room_number, phone, pickup_slot, "Pending Pickup"),
        )
